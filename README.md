...
- Prerender statico (SSG) con `vite-plugin-prerenderer` per route chiave (generazioni, pagine legali e Pokémon iconici).
...
## Prerender / SSG
- Prerendering statico tramite `vite-plugin-prerenderer`.
- Le route prerenderizzate sono definite in `vite.config.js`.
...
* Static prerender (SSG) with `vite-plugin-prerenderer` for key routes (generations, legal pages, iconic Pokémon).
...
## Prerender / SSG
* Static prerendering via `vite-plugin-prerenderer`.
* Prerender routes are defined in `vite.config.js`.
